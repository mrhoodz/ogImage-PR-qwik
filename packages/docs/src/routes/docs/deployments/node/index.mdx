...
It's _very important to correctly configure the `ORIGIN` env variable_, which is used to check against [CSRF attacks](https://owasp.org/www-community/attacks/csrf). The origin must match the origin of the client application.
