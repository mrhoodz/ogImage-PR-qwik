...
export default component$(() => {
  ...
  return (
    <div>
      ...
      <button onClick$={() => signal.value--}>-</button>
      <div>
        {isBiggerThanZero} - Current value: {signal.value}
      </div>
    </div>
  );
});
...
export default component$(() => {
  ...
  return (
    <div>
      ...
      <button onClick$={() => signal.value--}>-</button>
      <div>
        {signal.value > 0 ? 'Bigger than zero' : 'Smaller than zero'} - Current value:{' '}
        {signal.value}
      </div>
    </div>
  );
...
When using `useVisibleTask` to programatically register events, we are downloading and executing JavaScript eagerly, even if the event is not triggered.
