---
title: Request Handling | QwikCity
contributors:
  - adamdbradley
  - manucorporat
  - mhevery
...
Each route has the ability to add HTTP request and response handlers, allowing for developers to retrieve and modify data. The handlers can also be used by [endpoints](</docs/(qwikcity)/endpoints/index.mdx>), which only respond with data rather than a page's HTML.
