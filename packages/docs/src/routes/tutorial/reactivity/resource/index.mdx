...
Use `<Resource>` to display the data of the [`useResource$()`](</docs/(qwik)/components/state/index.mdx>) function. The `<Resource>` allows you to render different content depending if the resource is `pending`, `resolved` or `rejected`.
